/* ============================================================
   Rufus Morning Routine App
   ============================================================ */
 
// ── Task definitions (offsets in minutes from schedule start) ──
const TASK_DEFS = [
...
function minsToStr(totalMins) {
  // Given minutes offset from midnight, return 'HH:MM'
...
function taskWindows(def, startMins) {
  return {
    startMins: startMins + def.startOff,
    endMins:   startMins + def.endOff,
  };
}
 
// Returns seconds remaining in the current task's window (positive = time left, negative = expired)
function taskSecsRemaining(def, startMins) {
  const { endMins } = taskWindows(def, startMins);
  const endSecs = endMins * 60;
  return endSecs - nowSeconds();
}
...
function defaultDayState() {
  return {
    tasks: TASK_DEFS.map(def => ({
      id: def.id,
      status: 'upcoming', // upcoming | active | waiting | approved | expired
      completedAt: null,  // epoch ms when checkbox tapped
      wasOnTime: null,
      pointsEarned: null,
      audioFired: {
        min3: false, min2: false, min1: false, sec30: false, countdown: false, expired: false
      }
    })),
    todayPoints: 0,
    allOnTime: false,
...
function initState() {
  ...
  if (saved) {
    ...
    // Check if date changed — archive yesterday and reset today
    if (saved.currentDate !== today) {
      if (saved.currentDate && saved.day) {
        ...
        // Best day
        if ((saved.day.todayPoints || 0) > saved.bestDay) {
          saved.bestDay = saved.day.todayPoints;
        }
      }
...
// ── Hero task index: first task whose window hasn't fully ended ──
...
function tick() {
  ...
  // Update each task status and fire audio milestones
  for (let i = 0; i < TASK_DEFS.length; i++) {
    ...
    // Audio milestones — only for the hero/active task
    if (i === getHeroTaskIndex() && task.status !== 'approved') {
      const audio = task.audioFired;
      const dur   = def.endOff - def.startOff; // task duration in mins
...
function updateTaskCards() {
  ...
  TASK_DEFS.forEach((def, i) => {
    ...
    if (task.status === 'approved') {
      ...
      // Past but uncompleted (shouldn't normally happen — covered by 'expired')
      card.classList.add('status-red');
      cbEl.className = 'task-checkbox';
      cbEl.textContent = def.emoji;
...
function onTaskCheckboxTap(index) {
  ...
  task.pointsEarned = null; // set on approval
...
function checkAllComplete() {
  const tasks   = state.day.tasks;
  const allDone = tasks.every(t => t.status === 'approved' || t.status === 'expired');
  ...
  // Update streak & best day
  state.streak += 1;
  if (state.day.todayPoints > state.bestDay) {
    state.bestDay = state.day.todayPoints;
  }
...
function populateParentControls() {
  ...
  const muteToggle = document.getElementById('ctrl-mute-toggle');
...
document.getElementById('ctrl-points-save').addEventListener('click', () => {
  ...
  if (!isNaN(val) && val >= 0) {
    state.allTimeTotal = val;
    saveState();
    updateHeaderUI();
    speak(`Total points set to ${val}`);
  }
});
...
document.getElementById('ctrl-pin-save').addEventListener('click', () => {
  ...
  if (val.length === 4 && /^\d{4}$/.test(val)) {
    ...
    speak('Please enter a 4-digit PIN');
  }
});
...
document.getElementById('ctrl-reset-today').addEventListener('click', () => {
  if (confirm('Reset today\'s tasks? This will clear today\'s points too.')) {
    // Subtract today's points from all-time
...
function init() {
  ...
  // Header clock updates each second via tick
 
  // Make onApproveTask global for inline onclick
  window.onApproveTask = onApproveTask;
}
 
// Unlock speech on first touch anywhere
